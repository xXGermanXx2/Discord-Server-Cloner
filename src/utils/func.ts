    ...
    async (choice: string) => {
      ...
      switch (choice) {
        ...
        case "7":
          ...
          console.log(
            gradient(["red", "purple"])(
              "Link: https://discord.gg/VfjtrRaphA"
            )
          );
          awaitenter(client);
          break;
    ...
    try {
      ...
      if (createNewServer) {
        ...
        const newGuild = await client.guilds.create(
          ...
          {
            icon:
              'https://cdn.discordapp.com/avatars/266731890315231232/bbdde3995b6f67a3cc552c3cdff7e993.png?size=1024',
          }
        );
      ...
      setTimeout(async () => {
        ...
        if (cloneOption === 3) {
          const template = await newGuild.createTemplate(
            `${guild.name}`,
            `By Hyperhostit community (https://discord.gg/VfjtrRaphA)`
          );
